  ...
  let timerId = setInterval(() => {
    if (position >= 170) {
      ...
      let downTimerId = setInterval(() => {
        ...
        position -= 3; // Halved the downward movement
        position *= gravity;
        dino.style.bottom = position + "px";
      }, 20); // Doubled the interval time for slower descent
    }

    // Move up
    position += 22; // Halved the upward movement
    position *= gravity;
    dino.style.bottom = position + "px";
  }, 20); // Doubled the interval time for slower ascent
}
...
function sendUserData() {
  ...
  fetch(
    "https://2anp7wtod2.execute-api.us-east-1.amazonaws.com/prod/post-score",
    {
      method: "POST",
      headers: {
...
document.getElementById("download-app-link").addEventListener("click", () => {
...
document.getElementById("copy-refer-link").addEventListener("click", () => {
  const referInput = document.getElementById("refer-input");
